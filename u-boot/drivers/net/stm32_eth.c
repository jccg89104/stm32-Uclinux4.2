...
static struct stm32f2_gpio_dsc mac_gpio[] = {
	...
	{STM32F2_GPIO_PORT_B, 11},
	{STM32F2_GPIO_PORT_B, 12},
	{STM32F2_GPIO_PORT_B, 13},
...
/* PC13 as ETH reset*/
static struct stm32f2_gpio_dsc eth_reset_gpio = {STM32F2_GPIO_PORT_C, 13};
...
static  s32 stm_mac_hw_init_no_link(struct stm_eth_dev *mac);
...
static int stm32_mii_write(char *devname, u8 adr, u8 reg, u16 val);
static int stm32_mii_read(char *devname, u8 adr, u8 reg, u16 *val);
...
s32 stm32_eth_init(bd_t *bd)
{
	 ...
	 * Register PHY driver
	 */
	miiphy_register(STM32_MAC_NAME, stm32_mii_read, stm32_mii_write);
	printf("miiphy_register done.\n");

	/*
	 * Init hw
	 */
	stm32f2_gpio_config(&eth_reset_gpio, STM32F2_GPIO_ROLE_GPOUT);
	stm32f2_gpout_set(&eth_reset_gpio, 0);
	udelay(30000);
	stm32f2_gpout_set(&eth_reset_gpio, 1);
	udelay(30000);
	rv = stm_mac_hw_init_no_link(mac);
	if (rv != 0)
		goto out;
	printf("eth hw init done.\n");	
...
static int stm32_mii_write(char *devname, u8 adr, u8 reg, u16 val)
{
	...
	addr &= STM32_MAC_MIIAR_PA_MSK;
	tmp |= adr << STM32_MAC_MIIAR_PA_BIT;

	reg_num &= STM32_MAC_MIIAR_MR_MSK;
	tmp |= reg << STM32_MAC_MIIAR_MR_BIT;
...
static s32 stm_mac_gpio_init(struct stm_eth_dev *mac)
{
	 ...
	 * Enable MCO1 output
	 */
	//rv = stm32f2_gpio_config(&mco1_gpio, STM32F2_GPIO_ROLE_MCO);
	gpio_inited = 1;
	rv = 0;
out:
	return rv;
